...
[MANIFEST.md](./MANIFEST.md)「5. データモデリング」「6. API設計」を基に、実装コード（`backend/src/modules/*`）
...
- 【認可】[MANIFEST.md 2章の権限マトリクス](./MANIFEST.md)（全28エンドポイント）と実装のGuard/Service内チェックの対応が漏れなく取れているか、エンドポイント一覧と照らして横断チェックする。
